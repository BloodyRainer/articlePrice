...
func makeArticleQuestionFirstPlayer(dfReq df.Request, gs *df.GameSession) *df.Response {
	...
	payload := df.MakeSimpleRespPayload(true,
		"<speak>Wie ist der Preis von "+df.ModifyForTTS(gs.CurrentArticleName)+" auf otto <say-as interpret-as='characters'>DE</say-as>? <break time='300ms'/> "+pName+", du bist dran, für "+points+" Punkte, wie ist dein Tipp?</speak>",
